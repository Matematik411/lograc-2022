...
list-vec-list {A} = fun-ext check-lvl
   where
      check-lvl : (xs : List A) → (vec-list ∘ list-vec) xs ≡ id xs
      check-lvl [] = refl
      check-lvl (x ∷ xs) = cong (x ∷_) (check-lvl xs)
{-
...
Σ-assoc = record { 
   to = λ { ( x , ( y , z )) → (x , y) , z} ; 
   from = λ {((x , y ) , z) → x , (y , z)} ; 
   from∘to = λ { xyz → refl } ; -- this splitting can happen internally because it's a record type 
   to∘from = λ { xyz → refl } 
   }
